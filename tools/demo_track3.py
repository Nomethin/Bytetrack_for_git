...
#使用这个魔咒python tools/demo_track4.py --demo webcam --camid 0 --exp_file exps/example/mot/yolox_x_mix_det.py --ckpt pretrained/bytetrack_x_mot17.pth.tar --fp16 --fuse --save_result --target_id 1
...
def make_parser():
    ...
    # 保留可选参数的定义，删除位置参数的重复定义
    parser.add_argument("--demo", type=str, default="video", help="demo type, eg. image, video and webcam")
    parser.add_argument("--target_id", type=int, default=None, help="ID of the target person to track")  # target_id 参数

    parser.add_argument("-expn", "--experiment_name", type=str, default=None)
    parser.add_argument("-n", "--name", type=str, default=None, help="model name")
    parser.add_argument("--path", default="./videos/my_video.mp4", help="path to images or video")
...
def imageflow_demo(predictor, vis_folder, current_time, args):
    ...
    while True:
        ...
        if elapsed_time > 10:
            logger.info("达到了10秒时间限制，停止读取新帧，但继续处理已读取的帧。")
            break
            ...
            for t in online_targets:
                ...
                if tlwh[2] * tlwh[3] > args.min_box_area and not vertical:
                    ...
                    if tid == target_id:
                        ...
                        logger.info(f"为目标 {tid} 绘制方框，位置: {x1}, {y1}, 宽度: {w}, 高度: {h}")
